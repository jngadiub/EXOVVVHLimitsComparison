...
#  channels=["HbbZqqHPLP", "HbbWqqHPLP", "HwwZqqHPLPHV", "HwwWqqHPLPHV", "HbbWqqHwwHPLPHV", "HbbZqqHwwHPLPHV", "HWqq", "HZqq"] 
 ...
 #channels=[ "HWqq", "HZqq", "HbbZqqHPLP", "HbbWqqHPLP", "HwwZqqHPLPHV", "HwwWqqHPLPHV", "HbbWqqHwwHPLPHV", "HbbZqqHwwHPLPHV"] 
  ...
  for chan in channels:
    ...
    (a,b,c,d,e,f) = Plot( plots, chan, unblind )  
    if 'W' in chan :
    ...
    mg.Add( a, "PL" )
    mg.Add( b, "L" )
    a.SaveAs( "AllOn1Plot_%s_obs.root"%(chan) )
    b.SaveAs( "AllOn1Plot_%s_exp.root" %(chan) )

    print "before"
    print d.GetN()
    for p in range( c.GetN()-1, -1, -1 ):
       x = rt.Double(0.)
       y = rt.Double(0.)
       c.GetPoint(p,x,y)
       d.SetPoint(p+c.GetN()+1,x,y)
       print p,p+1+c.GetN(),x,y
   
    print "after"
    print d.GetN()       
    d.SaveAs("AllOn1Plot_%s_sigma2.root"%(chan))
...
#  channels=["HbbZqqHPLP", "HbbWqqHPLP", "HwwZqqHPLPHV", "HwwWqqHPLPHV", "HbbWqqHwwHPLPHV", "HbbZqqHwwHPLPHV", "HWqq", "HZqq"]
  mg.GetYaxis().SetTitle("#sigma #times B(V'->HV)" )
